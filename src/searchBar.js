  ...
  async function temp_func(search_param) {
    return await fetch(
      "https://api.geoapify.com/v1/geocode/autocomplete?text=" +
        search_param +
        "&lang=en&limit=5&type=state&format=json&apiKey=f2f56c29a7ca4cb38bdbda41130ae76e"
  ...
  const loadOptions = async (location) => {
    try {
      ...
      const options = responseData.results.map((city) => ({
        ...
        coord: [
          city.bbox ? (city.bbox.lat1 + city.bbox.lat2) / 2 : city.lat,
          city.bbox ? (city.bbox.lon1 + city.bbox.lon2) / 2 : city.lon,
        ],
      }));
      return { options };
    } catch (err) {
