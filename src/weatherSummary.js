...
function WeatherSummary({
  ...
  weather[0].description =
    weather[0].description[0].toUpperCase() + weather[0].description.substr(1);
  ...
  function getCurrentTimeStampIndex(hourlyForeCast) {
    for (let i = 0; i <= 24; i++) {
      if (
        hourlyForeCast.hourly.time[i].substring(0, 13) ===
        hourlyForeCast.current_weather.time.substring(0, 13)
      ) {
        return i;
      }
    }
  ...
  function HourlyForeCastComponent(props) {
    ...
    return (
      <div className="px-6 py-2">
        ...
        <img
          alt="weather-icon"
          src={`icons/${props.weatherIconID}${props.weatherIconIsDay}.png`}
        />
        <div className="text-black text-base mt-1">
          {celsius_to_unit(Math.floor(props.temp), unitValue)}{" "}
          {unitDisplay(unitValue)}
        </div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-center mt-4">
        <p className="text-black text-4xl font-bold">
          {kelvin_to_unit(main.temp, unitValue)} {unitDisplay(unitValue)}
        </p>
      </div>
      ...
      <div className="flex flex-row items-center justify-center">
        <p className="text-black text-lg">
          High:{" "}
          {celsius_to_unit(
            Math.floor(dailyTempData.daily.temperature_2m_max[0]),
            unitValue
          )}{" "}
          {unitDisplay(unitValue)} | Low:{" "}
          {celsius_to_unit(
            Math.floor(dailyTempData.daily.temperature_2m_min[0]),
            unitValue
          )}{" "}
          {unitDisplay(unitValue)}
        </p>
          ...
          <div className="flex flex-col w-full items-center justify-center">
            <div className="bg-card w-1/2 rounded-tl">
              <div className="mb-8">
                ...
                <p className="text-base">
                  {dailyTempData.daily.uv_index_max[0]}
                </p>
              </div>
            </div>
            <div className="bg-card w-1/2 rounded-tr">
              <div className="mb-8">
                ...
                <p className="text-base">
                  {dailyTempData.daily.sunrise[0].substring(11, 16)} A.M.
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-col w-full items-center justify-center">
            <div className="bg-card w-1/2 rounded-bl">
              <div className="mb-8">
                ...
                <p className="text-base">
                  {celsius_to_unit(
                    hourlyForeCast.hourly.apparent_temperature[
                      getCurrentTimeStampIndex(hourlyForeCast)
                    ],
                    unitValue
                  )}{" "}
                  {unitDisplay(unitValue)}
            ...
            <div className="bg-card w-1/2 rounded-br">
              <div className="mb-8">
                ...
                <p className="text-base">
                  {
                    hourlyForeCast.hourly.relativehumidity_2m[
                      getCurrentTimeStampIndex(hourlyForeCast)
                    ]
                  }
                  %
